...
class BaryScoreMetric:
    ...
    def evaluate_batch(self, batch_hyps, batch_refs, idf_hyps=None, idf_ref=None, show_tqdm=True):
        """
        ...
        with torch.no_grad():
            ...
            ####################################
            for index_sentence in tqdm(range(nb_sentences), 'BaryScore Progress', disable=not show_tqdm):
                dict_score = {}
